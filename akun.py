...
class AkunSiswa(Akun):
    ...
    def bayar(self, akunkantin, jumlah):
        if self.__saldo >= jumlah:
            ...
            self._riwayat.append(f"Pembayaran ke {akunkantin.nama}: \t -{jumlah}")
            self.list_transaksi.append(f"- \t{jumlah}")
...
class AkunKantin(Akun):
    ...
    def siswa_bayar(self, jumlah):
        ...
        self._riwayat.append(f"Potongan biaya admin: \t -2000")
        self.list_transaksi.append(f"+ \t{jumlah}")
        self.list_transaksi.append(f"- \t2000")
        jumlah -= 2000 # Potongan biaya admin
        self.__saldo += jumlah
